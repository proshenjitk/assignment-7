...
const updateTaxAndCharge = () => {
  ...
  if (priceConverted > 200) {
    ...
    const taxCal = priceConverted * 0.2;
    const setDigit = taxCal.toFixed(2);

    setInnerText("total-tax", setDigit);
  }
  if (priceConverted > 400) {
    setInnerText("delivery-charge", 50);

    const taxCalcu = priceConverted * 0.3;
    const setDigit2 = taxCalcu.toFixed(2);
    setInnerText("total-tax", setDigit2);
  }
  if (priceConverted > 500) {
    setInnerText("delivery-charge", 60);

    const taxCalculation = priceConverted * 0.4;
    const setDigit3 = taxCalculation.toFixed(2);
    setInnerText("total-tax", setDigit3);
  }
};
